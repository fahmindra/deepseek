...
![Benchmark performance of DeepSeek-V3 and its counterparts.](https://arxiv.org/html/2412.19437v2/x1.png)
...
2. [Architecture](#S2)
    2.1 [Basic Architecture](#S2.SS1)
        2.1.1 [Multi-Head Latent Attention](#S2.SS1.SSS1)
        2.1.2 [DeepSeekMoE with Auxiliary-Loss-Free Load Balancing](#S2.SS1.SSS2)
    2.2 [Multi-Token Prediction](#S2.SS2)
3. [Infrastructures](#S3)
    3.1 [Compute Clusters](#S3.SS1)
    3.2 [Training Framework](#S3.SS2)
        3.2.1 [DualPipe and Computation-Communication Overlap](#S3.SS2.SSS1)
        3.2.2 [Efficient Implementation of Cross-Node All-to-All Communication](#S3.SS2.SSS2)
        3.2.3 [Extremely Memory Saving with Minimal Overhead](#S3.SS2.SSS3)
    3.3 [FP8 Training](#S3.SS3)
        3.3.1 [Mixed Precision Framework](#S3.SS3.SSS1)
        3.3.2 [Improved Precision from Quantization and Multiplication](#S3.SS3.SSS2)
        3.3.3 [Low-Precision Storage and Communication](#S3.SS3.SSS3)
...
- [A Contributions and Acknowledgments](#A1)
- [B Ablation Studies for Low-Precision Training](#A2)
- [C Expert Specialization Patterns](#A3)
...
![Illustration of the basic architecture of DeepSeek-V3.](https://arxiv.org/html/2412.19437v2/x2.png)
...
$$
f_{i} = \frac{N_{r}}{K_{r}T} \sum_{t=1}^{T} \mathbb{1} \left(s_{i,t} \in \operatorname{Topk}(\{s_{j,t} | 1 \leqslant j \leqslant N_{r}\}, K_{r})\right), \tag{18}
$$
...
![Illustration of our Multi-Token Prediction (MTP) implementation.](https://arxiv.org/html/2412.19437v2/x3.png)
...
where $T$ represents the input sequence length and $_{i:j}$ denotes the slicing operation (inclusive of both the left and right boundaries). Finally, taking $\mathbf{h}_{i}^{k}$ as the input, the shared output head will compute the probability distribution for the $k$-th additional prediction token $P_{i+1+k}^{k}\in\mathbb{R}^{V}$, where $V$ is the vocabulary size:
...
<section id="S3.SS2.SSS1">

#### 3.2.1 DualPipe and Computation-Communication Overlap

![Overlapping strategy for a pair of individual forward and backward chunks.](https://arxiv.org/html/2412.19437v2/x4.png)
...
![Example DualPipe scheduling for 8 PP ranks and 20 micro-batches.](https://arxiv.org/html/2412.19437v2/x5.png)
...
![The overall mixed precision framework with FP8 data format.](https://arxiv.org/html/2412.19437v2/x6.png)
...
![Fine-grained quantization and high-precision accumulation.](https://arxiv.org/html/2412.19437v2/x7.png)
...
![Evaluation results on the NIAH tests.](https://arxiv.org/html/2412.19437v2/x8.png)
...
| LiveCodeBench-Base | 3-shot | 11.6 | 12.9 | 15.5 | **19.4** |
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| # Activated Params (Inf.) | - | 2.4B | 2.4B | 20.9B | 20.9B |
| # Total Params (Inf.) | - | 15.7B | 15.7B | 228.7B | 228.7B |
...
| Benchmark (Metric) | # Shots | Small MoE Aux-Based | Small MoE Aux-Free | Large MoE Aux-Based | Large MoE Aux-Free |
| :--- | :---: | :---: | :---: | :---: | :---: |
...
![Expert load of auxiliary-loss-free and auxiliary-loss-based models.](https://arxiv.org/html/2412.19437v2/x9.png)
...
&\frac{1}{G}\sum_{i=1}^{G}\left(\min\left(\frac{\pi_{\theta}(o_{i}|q)}{\pi_{\theta_{old}}(o_{i}|q)}A_{i},\text{clip}\left(\frac{\pi_{\theta}(o_{i}|q)}{\pi_{\theta_{old}}(o_{i}|q)},1-\varepsilon,1+\varepsilon\right)A_{i}\right)-\beta\mathbb{D}_{KL}\left(\pi_{\theta}||\pi_{ref}\right)\right),\end{split} \tag{26)
$$
...
| IF-Eval (Strict) | 57.7 | 80.6 | 84.1 | 86.0 | **86.5** | 84.3 | 86.1 |
| GPQA-Diamond | 35.3 | 41.3 | 49.0 | 51.1 | **65.0** | 49.9 | 59.1 |
| SimpleQA (Correct) | 9.0 | 10.2 | 9.1 | 17.1 | 28.4 | **38.2** | 24.9 |
| FRAMES (Acc.) | 66.9 | 65.4 | 69.8 | 70.0 | 72.5 | **80.5** | 73.3 |
| LongBench v2 (Acc.)| 31.6 | 35.4 | 39.4 | 36.1 | 41.0 | 48.1 | **48.7** |
| **Code** | | | | | | | |
| HumanEval-Mul | 69.3 | 77.4 | 77.3 | 77.2 | 81.7 | 80.5 | **82.6** |
| LCB (Pass@1-COT) | 18.8 | 29.2 | 31.1 | 28.4 | 36.3 | 33.4 | **40.5** |
| LCB (Pass@1) | 20.3 | 28.4 | 28.7 | 30.1 | 32.8 | 34.2 | **37.6** |
| Codeforces (Perc.) | 17.5 | 35.6 | 24.8 | 25.3 | 20.3 | 23.6 | **51.6** |
| SWE Verified | - | 22.6 | 23.8 | 24.5 | **50.8** | 38.8 | 42.0 |
| Aider-Edit (Acc.) | 60.3 | 71.6 | 65.4 | 63.9 | **84.2** | 72.9 | 79.7 |
| Aider-Polyglot | - | 18.2 | 7.6 | 5.8 | 45.3 | 16.0 | **49.6** |
| **Math** | | | | | | | |
| AIME 2024 (P@1) | 4.6 | 16.7 | 23.3 | 23.3 | 16.0 | 9.3 | **39.2** |
| MATH-500 (EM) | 56.3 | 74.7 | 80.0 | 73.8 | 78.3 | 74.6 | **90.2** |
| CNMO 2024 (P@1) | 2.8 | 10.8 | 15.9 | 6.8 | 13.1 | 10.8 | **43.2** |
| **Chinese** | | | | | | | |
| CLUEWSC (EM) | 89.9 | 90.4 | **91.4** | 84.7 | 85.4 | 87.9 | 90.9 |
| C-Eval (EM) | 78.6 | 79.5 | 86.1 | 61.5 | 76.7 | 76.0 | **86.5** |
| C-SimpleQA | 48.5 | 54.1 | 48.4 | 50.4 | 51.3 | 59.3 | **64.8** |
*Table 6: Comparison between DeepSeek-V3 and other representative chat models. All models are evaluated in a configuration that limits the output length to 8K. DeepSeek-V3 stands as the best-performing open-source model, and also exhibits competitive performance against frontier closed-source models.*
...
On Arena-Hard, DeepSeek-V3 achieves an impressive win rate of over 86% against the baseline GPT-4-0314, performing on par with top-tier models like Claude-Sonnet-3.5-1022. Similarly, DeepSeek-V3 showcases exceptional performance on AlpacaEval 2.0, outperforming both closed-source and open-source models.
...
During the development of DeepSeek-V3, we employ the constitutional AI approach, leveraging the voting evaluation results of DeepSeek-V3 itself as a feedback source. This method has produced notable alignment effects.

#### 5.4.3 Multi-Token Prediction Evaluation
DeepSeek-V3 predicts the next 2 tokens. Based on our evaluation, the acceptance rate of the second token prediction ranges between 85% and 90%. This high acceptance rate enables DeepSeek-V3 to achieve a significantly improved decoding speed, delivering 1.8 times TPS (Tokens Per Second) with speculative decoding.
...
In this paper, we introduce DeepSeek-V3, a large MoE language model with 671B total parameters and 37B activated parameters, trained on 14.8T tokens. DeepSeek-V3 has emerged as the strongest open-source model currently available, and achieves performance comparable to leading closed-source models like GPT-4o and Claude-3.5-Sonnet. It requires only 2.788M H800 GPU hours for its full training.

DeepSeek-V3 has some limitations: (1) The recommended deployment unit is relatively large. (2) There is still potential for further enhancement of generation speed.

In the future, we plan to:
*   Refine model architectures to approach infinite context length and break through Transformer limitations.
*   Continuously iterate on data quantity and quality.
*   Explore deep thinking capabilities to enhance reasoning length and depth.
*   Explore more comprehensive evaluation methods.
...
- Bai, Y., et al. LongBench v2, 2024.
- Bauer, M., et al. Singe: leveraging warp specialization for high performance on GPUs, 2014.
- Bisk, Y., et al. PIQA: reasoning about physical commonsense in natural language, 2020.
- Chen, M., et al. Evaluating large language models trained on code, 2021.
- Clark, P., et al. Think you have solved question answering? try arc, 2018.
- Cobbe, K., et al. Training verifiers to solve math word problems, 2021.
- Cui, Y., et al. A span-extraction dataset for Chinese machine reading comprehension, 2019.
- Dai, D., et al. Deepseekmoe: Towards ultimate expert specialization, 2024.
- DeepSeek-AI. Deepseek-coder-v2, 2024a.
- DeepSeek-AI. Deepseek LLM, 2024b.
- DeepSeek-AI. Deepseek-v2, 2024c.
- Dettmers, T., et al. Gpt3. int8 (): 8-bit matrix multiplication, 2022.
- Ding, H., et al. Fewer truncations improve language modeling, 2024.
- Dua, D., et al. DROP, 2019.
- Dubois, Y., et al. Length-controlled alpacaeval, 2024.
- Fedus, W., et al. Switch transformers, 2021.
- Fishman, M., et al. Scaling FP8 training to trillion-token llms, 2024.
- Frantar, E., et al. Gptq, 2022.
- Gao, L., et al. The Pile, 2020.
- Gema, A. P., et al. Are we done with mmlu?, 2024.
- Gloeckle, F., et al. Better & faster large language models via multi-token prediction, 2024.
- Google. Gemini 1.5, 2024.
- Graham, R. L., et al. Scalable hierarchical aggregation protocol (SHArP), 2016.
- Gu, A., et al. Cruxeval, 2024.
- Guo, D., et al. Deepseek-coder, 2024.
- Harlap, A., et al. Pipedream, 2018.
- He, B., et al. Understanding and minimising outlier features, 2024.
- He, Y., et al. Chinese simpleqa, 2024.
- Hendrycks, D., et al. Measuring massive multitask language understanding, 2020.
- Hendrycks, D., et al. Measuring mathematical problem solving, 2021.
- Huang, Y., et al. C-Eval, 2023.
- Jain, N., et al. Livecodebench, 2024.
- Jiang, A. Q., et al. Mistral 7b, 2023.
- Joshi, M., et al. TriviaQA, 2017.
- Kalamkar, D., et al. A study of bfloat16 for deep learning training, 2019.
- Krishna, S., et al. Fact, fetch, and reason, 2024.
- Kwiatkowski, T., et al. Natural questions, 2019.
- Lai, G., et al. RACE, 2017.
- Lambert, N., et al. Rewardbench, 2024.
- Lepikhin, D., et al. Gshard, 2021.
- Leviathan, Y., et al. Fast inference from transformers via speculative decoding, 2023.
- Li, H., et al. CMMLU, 2023.
- Li, S., and Hoefler, T. Chimera, 2021.
- Li, T., et al. Arena-hard and benchbuilder pipeline, 2024a.
- Li, W., et al. Ccpm, 2021.
- Li, Y., et al. EAGLE, 2024b.
- Lin, B. Y. ZeroEval, 2024.
- Loshchilov, I., and Hutter, F. Decoupled weight decay regularization, 2017.
- Lundberg, S. The art of prompt design: Prompt boundaries and token healing, 2023.
- Luo, Y., et al. Ascend HiFloat8 format, 2024.
- MAA. American invitational mathematics examination - aime, 2024.
- Micikevicius, P., et al. FP8 formats for deep learning, 2022.
- Mistral. Mixtral-8x22b, 2024.
- Narang, S., et al. Mixed precision training, 2017.
- Noune, B., et al. 8-bit numerical formats, 2022.
- NVIDIA. Improving network performance using NVSHMEM, 2022.
- NVIDIA. Blackwell architecture, 2024a.
- NVIDIA. TransformerEngine, 2024b.
- OpenAI. Hello GPT-4o, 2024a.
- OpenAI. MMMLU, 2024b.
- OpenAI. Introducing SimpleQA, 2024c.
- OpenAI. Introducing SWE-bench verified, 2024d.
- Peng, B., et al. Yarn, 2023a.
- Peng, H., et al. FP8-LM, 2023b.
- Qi, P., et al. Zero bubble pipeline parallelism, 2023a.
- Qwen. Qwen technical report, 2023.
- Qwen. Qwen1.5, 2024a.
- Qwen. Qwen2.5, 2024b.
- Rajbhandari, S., et al. Zero: Memory optimizations, 2020.
- Rein, D., et al. GPQA, 2023.
- Rouhani, B. D., et al. Microscaling data formats, 2023a, b.
- Sakaguchi, K., et al. Winogrande, 2019.
- Shao, Z., et al. Deepseekmath, 2024.
- Shazeer, N., et al. Outrageously large neural networks, 2017.
- Shi, F., et al. Language models are multilingual chain-of-thought reasoners, 2023.
- Shibata, Y., et al. Byte pair encoding, 1999.
- Su, J., et al. Roformer, 2024.
- Sun, K., et al. CMRC, 2019a.
- Sun, M., et al. Massive activations in large language models, 2024.
- Sun, X., et al. Hybrid 8-bit floating point (HFP8), 2019b.
- Suzgun, M., et al. Challenging big-bench tasks, 2022.
- Thakkar, V., et al. CUTLASS, 2023.
- Touvron, H., et al. LLaMA, 2023a.
- Touvron, H., et al. Llama 2, 2023b.
- Vaswani, A., et al. Attention is all you need, 2017.
- Wang, L., et al. Auxiliary-loss-free load balancing strategy, 2024a.
- Wang, Y., et al. Mmlu-pro, 2024b.
- Wei, T., et al. Cmath, 2023.
- Wortsman, M., et al. Stable and low-precision training, 2023.
- Xi, H., et al. Training transformers with 4-bit integers, 2023.
- Xia, C. S., et al. Agentless, 2024.
- Xia, T., et al. Speculative decoding, 2023.
- Xiao, G., et al. Smoothquant, 2023.
- Xu, L., et al. CLUE, 2020.
- Zellers, R., et al. HellaSwag, 2019.
- Zhong, W., et al. AGIEval, 2023.
- Zhou, J., et al. Instruction-following evaluation, 2023.
...
**Business & Compliance**
Dongjie Ji, Jian Liang, Jin Chen, Leyi Xia, Miaojun Wang, Mingming Li, Peng Zhang, Shaoqing Wu, Shengfeng Ye, T. Wang, W.L. Xiao, Wei An, Xianzu Wang, Xinxia Shan, Ying Tang, Yukun Zha, Yuting Yan, Zhen Zhang.
...
![Loss curves comparison between BF16 and FP8 training.](https://arxiv.org/html/2412.19437v2/x10.png)
...
![Expert load layers 1-7](https://arxiv.org/html/2412.19437v2/x11.png)
*(a) Layers 1-7*

![Expert load layers 7-13](https://arxiv.org/html/2412.19437v2/x12.png)
*(b) Layers 7-13*

![Expert load layers 13-19](https://arxiv.org/html/2412.19437v2/x13.png)
*(c) Layers 13-19*

![Expert load layers 19-25](https://arxiv.org/html/2412.19437v2/x14.png)
*(d) Layers 19-25*

![Expert load layers 25-27](https://arxiv.org/html/2412.19437v2/x15.png)
*(e) Layers 25-27*
