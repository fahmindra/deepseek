...
![Screenshot for page 1](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/1.png)
...
![Screenshot for page 2](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/2.png)
...
![Screenshot for page 3](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/3.png)
...
![Screenshot for page 4](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/4.png)
...
![Screenshot for page 5](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/5.png)
...
![Screenshot for page 6](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/6.png)
...
![Screenshot for page 7](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/7.png)
...
**Standard Hyper-Connections.** The standard HC expands the width of the residual stream by a factor of $n_{\text{hc}}$. Specifically, the shape of the residual stream is expanded from $\mathbb{R}^d$ to $\mathbb{R}^{n_{\text{hc}} \times d}$, where $d$ is the hidden size of the actual layer input. Let $X_l = [\mathbf{x}_{l,1}; \dots; \mathbf{x}_{l,n_{\text{hc}}}]^T \in \mathbb{R}^{n_{\text{hc}} \times d}$ be the residual state before the $l$-th layer. HC introduces three linear mappings: an input mapping $A_l \in \mathbb{R}^{1 \times n_{\text{hc}}}$, a residual transformation $B_l \in \mathbb{R}^{n_{\text{hc}} \times n_{\text{hc}}}$, and an output mapping $C_l \in \mathbb{R}^{n_{\text{hc}} \times 1}$. The update of the residual state is then formulated as:
...
![Screenshot for page 8](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/8.png)
...
**Dynamic Parameterization.** The parameters of three linear mappings are dynamically generated, which are decomposed into a dynamic (input-dependent) component and a static (input-independent) component. Given the input $X_l \in \mathbb{R}^{n_{\text{hc}} \times d}$, it is first flattened and normalized: $\hat{X}_l = \text{RMSNorm}(\text{vec}(X_l)) \in \mathbb{R}^{1 \times n_{\text{hc}}d}$. Then, we follow the conventional HC to generate the unconstrained raw parameters $\tilde{A}_l \in \mathbb{R}^{1 \times n_{\text{hc}}}$, $\tilde{B}_l \in \mathbb{R}^{n_{\text{hc}} \times n_{\text{hc}}}$, and $\tilde{C}_l \in \mathbb{R}^{n_{\text{hc}} \times 1}$:
...
where $W_l^{\text{pre}}, W_l^{\text{post}} \in \mathbb{R}^{n_{\text{hc}}d \times n_{\text{hc}}}$ and $W_l^{\text{res}} \in \mathbb{R}^{n_{\text{hc}}d \times n_{\text{hc}}^2}$ are learnable parameters for generating the dynamic components; $\text{Mat}(\cdot)$ reshapes a vector of size $1 \times n_{\text{hc}}^2$ into a matrix of size $n_{\text{hc}} \times n_{\text{hc}}$; $S_l^{\text{pre}} \in \mathbb{R}^{1 \times n_{\text{hc}}}$, $S_l^{\text{post}} \in \mathbb{R}^{n_{\text{hc}} \times 1}$, and $S_l^{\text{res}} \in \mathbb{R}^{n_{\text{hc}} \times n_{\text{hc}}}$ are learnable static biases; and $\alpha_l^{\text{pre}}, \alpha_l^{\text{res}}, \alpha_l^{\text{post}} \in \mathbb{R}$ are learnable gating factors initialized to small values.
...
where $\mathcal{T}_r$ and $\mathcal{T}_c$ denote row and column normalization, respectively. This iteration converges to a constrained doubly stochastic matrix $B_l = M^{(t_{\text{max}})}$. We choose $t_{\text{max}} = 20$ as a practical value.

***

![Screenshot for page 9](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/9.png)
...
![Screenshot for page 10](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/10.png)
...
$$[S^a_{m i : m(i+1)-1}; S^b_{m(i-1) : m i-1}] = \text{Softmax}_{\text{row}}([Z^a_{m i : m(i+1)-1} + B^a; Z^b_{m(i-1) : m i-1} + B^b]), \quad (11)$$
$$C_i^{\text{Comp}} = \sum_{j=mi}^{m(i+1)-1} S_j^a \odot C_j^a + \sum_{j=m(i-1)}^{mi-1} S_j^b \odot C_j^b, \quad (12)$$

where $\odot$ denotes the Hadamard product; $\text{Softmax}_{\text{row}}(\cdot)$ denotes the softmax operation along the row dimension, which performs normalization across the total of $2m$ elements from both $Z^a$ and $Z^b$. When $i=0$, $Z^b_{m(i-1):mi-1}$ is padded with negative infinity and $C^b_{m(i-1):mi-1}$ is padded with zeros. Note that each $C_i^{\text{Comp}}$ is derived from $2m$ KV entries, but the indexes of $C^b$ used for $C_i^{\text{Comp}}$ and the indexes of $C^a$ used for $C_{i-1}^{\text{Comp}}$ are overlapped. Therefore, CSA in fact compresses the sequence length to $\frac{1}{m}$ times.
...
$$I_{t,s} = \sum_{h=1}^{n_h^I} w_{t,h}^I \cdot \text{ReLU} \left( \mathbf{q}_{t,h}^I \cdot K_s^{\text{IComp}} \right), \quad (16)$$

where $W^w \in \mathbb{R}^{d \times n_h^I}$ is a learnable matrix; $w_{t,h}^I \in \mathbb{R}$ is the weight of the $h$-th indexer head. For a query token $t$, given its index scores $I_{t,:}$, we employ a top-k selector to selectively retain a subset of compressed KV entries $C_t^{\text{SprsComp}}$ for subsequent core attention:

$$C_t^{\text{SprsComp}} = \left\{ C_s^{\text{Comp}} \;\middle|\; I_{t,s} \in \text{Top-k}(I_{t,:}) \right\}. \quad (17)$$

***

![Screenshot for page 11](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/11.png)
...
where $n_h$ denotes the number of query heads; $W^{UQ} \in \mathbb{R}^{d_c \times c n_h}$ is the up-projection matrices for queries. Note that the latent query vector $\mathbf{c}_t^Q$ is shared with that used for the indexer queries. Next, we perform MQA on $\{\mathbf{q}_{t,i}\}$ and $C_t^{\text{SprsComp}}$:

$$\mathbf{o}_{t,i} = \text{CoreAttn} \left( \text{query=}\mathbf{q}_{t,i}, \text{key=}C_t^{\text{SprsComp}}, \text{value=}C_t^{\text{SprsComp}} \right), \quad (19)$$
...
![Screenshot for page 12](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/12.png)
...
![Screenshot for page 13](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/13.png)
...
**Attention Sink.** In the core attention of CSA and HCA, we employ the trick of attention sink (OpenAI, 2025; Xiao et al., 2024). To be specific, we set a series of learnable sink logits $\{z_1', z_2', \dots, z_{n_h}'\}$. For the $h$-th attention head, $\text{Exp}(z_h')$ will be added to the denominator of the attention score:

$$s_{h,i,j} = \frac{\text{Exp}(z_{h,i,j})}{\sum_k \text{Exp}(z_{h,i,k}) + \text{Exp}(z_h')}, \quad (27)$$
...
![Screenshot for page 14](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/14.png)
...
3:         $G_t = \nabla_W \mathcal{L}_t (W_{t-1})$ $\triangleright$ Compute gradients
4:         $M_t = \mu M_{t-1} + G_t$ $\triangleright$ Accumulate momentum buffer
5:         $O'_t = \text{HybridNewtonSchulz}(\mu M_t + G_t)$ $\triangleright$ Nesterov trick and hybrid Newton-Schulz
6:         $O_t = O'_t \cdot \sqrt{\max(n, m)} \cdot \gamma$ $\triangleright$ Rescale the update RMS
7:         $W_t = W_{t-1} \cdot (1 - \eta \lambda) - \eta O_t$ $\triangleright$ Perform weight decay and update
8:     **end for**
9: **end for**
...
![Screenshot for page 15](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/15.png)
...
![Screenshot for page 16](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/16.png)
...
![Screenshot for page 17](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/17.png)
...
![Screenshot for page 18](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/18.png)
...
![Screenshot for page 19](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/19.png)
...
![Screenshot for page 20](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/20.png)
...
![Screenshot for page 21](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/21.png)
...
![Screenshot for page 22](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/22.png)
...
![Screenshot for page 23](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/23.png)
...
![Screenshot for page 24](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/24.png)
...
![Screenshot for page 25](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/25.png)
...
![Screenshot for page 27](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/27.png)
...
![Screenshot for page 28](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/28.png)
...
| **World Knowl.** | | | | |
| AGIEval (EM) | 0-shot | 80.1 | <u>82.6</u> | **83.1** |
...
| **Code & Math** | | | | |
| BigCodeBench (Pass@1) | 3-shot | **63.9** | 56.8 | <u>59.2</u> |
...
![Screenshot for page 29](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/29.png)
...
![Screenshot for page 30](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/30.png)
...
![Screenshot for page 31](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/31.png)
...
![Screenshot for page 32](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/32.png)
...
![Screenshot for page 33](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/33.png)
...
$\{\pi_{E_1}, \pi_{E_2}, \dots, \pi_{E_N}\}$, the OPD objective function is defined as:
...
![Screenshot for page 34](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/34.png)
...
![Screenshot for page 35](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/35.png)
...
![Screenshot for page 36](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/36.png)
...
![Screenshot for page 37](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/37.png)
...
![Screenshot for page 38](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/38.png)

##### 5.3.2. Evaluation Results
Table 6 | Comparison between DeepSeek-V4-Pro-Max and closed/open source models. "Max", "xHigh", and "High" denote reasoning effort. The best results are highlighted in **bold**; the second-best results are <u>underlined</u>.
...
![Screenshot for page 39](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/39.png)
...
![Screenshot for page 40](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/40.png)
...
![Screenshot for page 41](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/41.png)
...
![Screenshot for page 42](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/42.png)
...
![Screenshot for page 43](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/43.png)
...
![Screenshot for page 44](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/44.png)
...
![Screenshot for page 45](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/45.png)
...
Y. Bai, Y. Bao, G. Chen, J. Chen, N. Chen, R. Chen, Y. Chen, Y. Chen, Z. Chen, J. Cui, H. Ding, M. Dong, A. Du, C. Du, D. Du, Y. Du, Y. Fan, Y. Feng, K. Fu, B. Gao, H. Gao, P. Gao, T. Gao, X. Gu, L. Guan, H. Guo, J. Guo, H. Hu, X. Hao, T. He, W. He, W. He, C. Hong, Y. Hu, Z. Hu, W. Huang, Z. Huang, Z. Huang, T. Jiang, Z. Jiang, X. Jin, Y. Kang, G. Lai, C. Li, F. Li, H. Li, M. Li, W. Li, Y. Li, Y. Li, Z. Li, Z. Li, H. Lin, X. Lin, Z. Lin, C. Liu, C. Liu, H. Liu, J. Liu, J. Liu, L. Liu, S. Liu, T. Y. Liu, T. Liu, W. Liu, Y. Liu, Y. Liu, Y. Liu, Y. Liu, Y. Liu, Z. Liu, E. Lu, L. Lu, S. Ma, X. Ma, Y. Ma, S. Mao, J. Mei, X. Men, Y. Miao, S. Pan, Y. Peng, R. Qin, B. Qu, Z. Shang, L. Shi, S. Shi, F. Song, J. Su, Z. Su, X. Sun, F. Sung, H. Tang, J. Tao, Q. Teng, C. Wang, D. Wang, F. Wang, and H. Wang. Kimi K2: open agentic intelligence. CoRR, abs/2507.20534, 2025a. URL `https://doi.org/10.48550/arXiv.2507.20534`.
...
![Screenshot for page 46](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/46.png)
...
![Screenshot for page 47](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/47.png)
...
![Screenshot for page 48](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/48.png)
...
![Screenshot for page 49](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/49.png)
...
![Screenshot for page 50](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/50.png)
...
![Screenshot for page 51](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/51.png)
...
![Screenshot for page 52](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/52.png)
...
![Screenshot for page 53](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/53.png)
...
![Screenshot for page 54](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/54.png)
...
![Screenshot for page 55](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/55.png)
...
| **Objective<br>Q&A**<br>(客观问答) | Single-value Search (单值信息查找) | 95 | 36 | 10 | 49 | 37.9 | 10.5 | 51.6 |
...
| **Subjective<br>Q&A**<br>(主观问答) | Causal Analysis (原因分析) | 100 | 28 | 5 | 67 | 28.0 | 5.0 | 67.0 |
...
![Screenshot for page 56](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/56.png)
...
![Screenshot for page 57](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/57.png)
...
| **Business<br>Writing**<br>(办公文本) | Report (报告) | 527 | 350 | 162 | 15 | 66.41 | 30.74 | 2.85 |
...
| **Media<br>Writing**<br>(媒体文本) | Social Media (社交媒体文案) | 267 | 156 | 101 | 10 | 58.43 | 37.83 | 3.75 |
...
| **Everyday<br>Writing**<br>(生活文本) | Congratulatory (祝贺文本) | 101 | 54 | 41 | 6 | 53.47 | 40.59 | 5.94 |
...
| **Oral<br>Writing**<br>(口头文本) | Speech (发言稿) | 226 | 135 | 85 | 6 | 59.73 | 37.61 | 2.65 |
...
| **Official<br>Document**<br>(公文文本) | Administrative Doc (事务文书) | 117 | 60 | 53 | 4 | 51.28 | 45.30 | 3.42 |
...
| **Academic<br>Writing**<br>(学术文本) | Research Paper (学术论文) | 104 | 67 | 32 | 5 | 64.42 | 30.77 | 4.81 |
...
![Screenshot for page 58](https://browser-tools.s3.us-west-2.amazonaws.com/1741271109012-z94pydn1s9/58.png)
